(function () {
    ...
    window['TVC'] = {
        ...
        showTip: function (msg, time, autohide, callback) {
            ...
            var html = '<div id="_tmp_pop_message" style="position:fixed; top: 45%;  width:200px; text-align:center; background:rgba(0,0,0,.7); line-height:25px; color:#fff; padding:8px 15px; z-index:10002;-webkit-border-radius:6px; -moz-border-radius: 6px;  border-radius: 6px;">' + msg + '</div>';
        ...
        relativeConfirm: function (msg, yes, no) {
            if ($("#_tmp_confirm_message").length > 0) {
                $("#_tmp_congirm_message").remove();
            }
            var left = document.documentElement.clientWidth / 2 - 100 + 'px';
        ...
        refresh:function(){
            $("body").on('click','input[data-refresh]',function(){
